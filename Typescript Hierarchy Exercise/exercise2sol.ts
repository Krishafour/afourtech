...
// • Differentiate type aliases from interfaces

 {
    ...
    */
    //we just create a interface with Cartitem with optional member variantId
    interface CartItem {
        id: number;
        title: string;
        variantId?: number;
      ...
      addToCart({ id: 1, title: 'Concrete shoes' })//variantId is optional
      addToCart({ id: 1, title: 'Concrete shoes',variantId:410 })
      // ======== Exercise 2.2 ========
      // Instructions:
    ...
    */
     interface City {
      ...
      // [/do not edit]
      //we are creating coords interface
    interface coords{
        latitude:number;
        longitude:number;
    }
    //exentending city interface by adding member of coords interface 
    interface City{
        coords:coords;
    }
      const montreal = {
    ...
    interface UserSchema {
      ...
      class User implements UserSchema {
        constructor(public name: string, readonly id: number) {
            this.id=id;//readonly can initialize in constructor
        }
      }
